...
def extract_patent_parts(patent_num_str):
    ...
    # 根据规则获取小类：大组内倒数第三个字符及其之前的字符
    sub_class = main_group[:-2] if len(main_group) > 2 else main_group
...
def process_row(row):
    ...
    for block_content in patent_blocks_content:
        ...
        if p > 0:
            ...
            method1_q_list.append(None) # 如果p=0，添加None
...
def main():
    ...
    print(f"所有行处理完毕，耗时: {process_time - load_time:.2f} 秒。")
